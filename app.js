...
function bindEvents() {
  ...
  if (el.bookingSearch) {
    el.bookingSearch.addEventListener("input", () => {
      state.bookingSearch = el.bookingSearch.value.trim().toLowerCase();
      renderBookingList();
    });
  }
  if (el.ticketSearch) {
    el.ticketSearch.addEventListener("input", () => {
      state.ticketSearch = el.ticketSearch.value.trim().toLowerCase();
      renderTicketList();
    });
  }
}
...
function renderBookingList() {
  const bookings = filterBookingsByName(getBookingsByDate(), state.bookingSearch);
  if (!bookings.length) {
    el.bookingList.innerHTML = state.bookingSearch ? "<p>Nessuna prenotazione trovata.</p>" : "<p>Nessuna prenotazione presente.</p>";
    return;
  }

  el.bookingList.innerHTML = renderDayGroups(bookings, (dayBookings) => {
    const people = peopleForBookings(dayBookings);
    const freeSeats = Math.max(0, getAvailabilityLimit(dayBookings[0].date) - people);
    return `
...
function renderProductSummary() {
  ...
  el.productSummary.innerHTML = renderDayGroups(bookings, (dayBookings) => {
    const people = peopleForBookings(dayBookings);
...
function renderTicketList() {
  ...
  const tickets = filterBookingsByName(
    getBookingsByDate().filter((booking) => booking.bookingType === "Online" && booking.ticketCode),
    state.ticketSearch
  );

  if (!tickets.length) {
    el.ticketList.innerHTML = state.ticketSearch ? "<p>Nessun ticket trovato.</p>" : "<p>Nessun ticket online presente.</p>";
    return;
  }

  el.ticketList.innerHTML = renderDayGroups(tickets, (dayTickets) => dayTickets.map((booking) => `
...
function filterBookingsByName(bookings, search) {
  ...
  return bookings.filter((booking) => String(booking.fullName || "").toLowerCase().includes(search));
}
