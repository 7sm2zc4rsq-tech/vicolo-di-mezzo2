...
function renderAdmin() {
  ...
  const nextDate = ALLOWED_DATES.find((date) => date >= today) || ALLOWED_DATES[0];
  const maxSeats = getAvailabilityLimit(nextDate);
  const usedSeats = seatsForDate(nextDate);
...
function renderBookingList() {
  ...
  el.bookingList.innerHTML = renderDayGroups(bookings, (dayBookings) => {
    const people = dayBookings.reduce((sum, booking) => sum + Number(booking.people || 0), 0);
    return `
      <p class="day-total">${dayBookings.length} prenotazioni - ${people} persone</p>
