...
function bindEvents() {
  ...
  document.querySelector("[data-admin-link]").addEventListener("click", (event) => {
...
function renderBookingForm(form, type) {
  ...
  form.innerHTML = `
    ...
    <label>Orario <input name="time" type="time" required></label>
...
async function handleAvailabilitySubmit(event) {
  ...
  const item = {
    date,
    maxSeats: Number(data.get("maxSeats")),
    updatedAt: new Date(),
...
function renderAdmin() {
  ...
  const maxSeats = state.availability.get(selectedDate)?.maxSeats;
...
function renderBookingList(date) {
  ...
  el.bookingList.querySelectorAll("[data-edit]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      if (people > 0 && await checkAvailability(booking.date, people, booking.id)) {
...
async function checkAvailability(date, people, excludedBookingId) {
  const limit = state.availability.get(date)?.maxSeats;
