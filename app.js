...
function renderTicketList() {
  ...
  const tickets = state.bookings
    .filter((booking) => booking.bookingType === "Online" && booking.ticketCode)
    .sort((a, b) => `${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`));
  ...
  el.ticketList.innerHTML = tickets.map((booking) => `
    <article class="ticket-card">
      <strong>${booking.ticketCode}</strong>
      <span>${booking.fullName}</span>
      <span>${formatLongDate(booking.date)} - ${booking.time}</span>
