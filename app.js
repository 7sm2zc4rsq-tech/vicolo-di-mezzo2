import { firebaseConfig } from "./firebase-config.js?v=20260604-fix2";

const ADMIN_EMAILS = [
  "fveronica73@gmail.com",
  "fspagnoli02@gmail.com",
  "alessandro.ciampi64@outlook.com"
];

const STAFF_PASSWORD = "Proloco@(16D)";

const DEFAULT_DAILY_CAPACITY = 120;
...
const CALENDAR_DATES = ALLOWED_DATES;

const SPECIAL_EVENTS = {
  "2026-07-31": {
    key: "trippaRomana",
    name: "Trippa romana",
    label: "Serata speciale: Trippa romana"
  },
  "2026-08-07": {
    key: "fagioliCotiche",
    name: "Fagioli con le cotiche",
    label: "Serata speciale: Fagioli con le cotiche"
  }
};
...
const MENU_ITEMS = [
  ...
  },
  {
    key: "trippaRomana",
    name: "Trippa romana",
    description: "Specialita disponibile solo venerdi 31 luglio",
    dateOnly: "2026-07-31"
  },
  {
    key: "fagioliCotiche",
    name: "Fagioli con le cotiche",
    description: "Specialita disponibile solo venerdi 7 agosto",
    dateOnly: "2026-08-07"
  }
];
...
const el = {
  ...
  ticketReceipt: document.querySelector("#ticketReceipt"),
  ...
  phoneBookingMessage: document.querySelector("#phoneBookingMessage"),
  phoneBookingPanel: document.querySelector("#phoneBookingPanel"),
...
function bindEvents() {
  ...
  if (el.adminDateFilter) {
    el.adminDateFilter.addEventListener("change", renderAdmin);
  }
  document.querySelectorAll("[data-section-toggle]").forEach((button) => {
    button.addEventListener("click", () => toggleAdminSection(button.dataset.sectionToggle));
  });
}
...
function renderPublicContent() {
  el.menuGrid.innerHTML = MENU_ITEMS.filter((item) => !item.dateOnly).map((item) => `
...
function renderBookingForm(form, type) {
  const menuControls = MENU_ITEMS.map((item) => `
    <article class="quantity-item ${item.dateOnly ? "is-date-only" : ""}" data-menu-item="${item.key}" ${item.dateOnly ? `data-date-only="${item.dateOnly}" hidden` : ""}>
      ${item.image ? `<img src="${item.image}" alt="${item.name}">` : `<div class="special-food-icon" aria-hidden="true">Specialita</div>`}
      <div>
        <strong>${item.name}</strong>
        ${item.dateOnly ? `<p class="special-menu-note">${item.description}</p>` : ""}
  ...
  form.innerHTML = `
    ...
    <label>Orario
      <select name="time" required>
        ...
        ${BOOKING_TIMES.map((time) => `<option value="${time}">${time}</option>`).join("")}
      </select>
    </label>
    <p class="form-hint full">Orari prenotabili solo dalle 19:30 alle 21:30.</p>
    ...
    <p class="special-selected-note full" data-special-selected hidden></p>
  ...
  updateSpecialMenuAvailability(form);
...
function renderDateCalendar(form) {
  ...
  calendar.innerHTML = CALENDAR_DATES.map((date) => {
    ...
    const specialEvent = SPECIAL_EVENTS[date];
...
function updateSpecialMenuAvailability(form) {
  const selectedDate = form.querySelector("[name='date']")?.value || "";
  const specialEvent = SPECIAL_EVENTS[selectedDate];
  const note = form.querySelector("[data-special-selected]");
  if (note) {
    note.hidden = !specialEvent;
    note.textContent = specialEvent ? specialEvent.label : "";
  }

  form.querySelectorAll("[data-date-only]").forEach((itemNode) => {
    const isAvailable = itemNode.dataset.dateOnly === selectedDate;
    itemNode.hidden = !isAvailable;
    if (!isAvailable) {
      const input = itemNode.querySelector("input[type='hidden']");
      const output = itemNode.querySelector("output");
...
async function handleBookingSubmit(event) {
  ...
  const message = form === el.bookingForm ? el.bookingMessage : el.phoneBookingMessage;
  ...
  if (booking.bookingType === "Online") {
    booking.ticketCode = createTicketCode();
    booking.ticketPresented = false;
  }
  ...
  if (form.dataset.bookingType === "Telefonica") {
    await refreshData();
    message.textContent = "Prenotazione telefonica aggiunta.";
    hideTicketReceipt();
  } else {
    resetMessage(message);
    showTicketReceipt(savedBooking);
    await refreshData();
  }
}
...
function formToBooking(form) {
  ...
  const menu = Object.fromEntries(MENU_ITEMS.map((item) => {
    const available = !item.dateOnly || item.dateOnly === date;
    return [item.key, menuChoice === "now" && available ? Number(data.get(item.key) || 0) : 0];
  }));
  return {
...
async function refreshData() {
  if (state.localMode) {
    ...
    const { collection, getDocs } = state.firebase;
    const bookingSnap = await getDocs(collection(state.firebase.db, "bookings"));
    const availabilitySnap = await getDocs(collection(state.firebase.db, "availability"));
    state.bookings = bookingSnap.docs
...
async function saveBooking(booking) {
  if (state.localMode) {
    ...
    const savedBooking = { ...booking, id: crypto.randomUUID() };
    bookings.push(savedBooking);
    writeLocal("bookings", bookings);
    return savedBooking;
  }
  const { addDoc, collection, serverTimestamp } = state.firebase;
  const docRef = await addDoc(collection(state.firebase.db, "bookings"), {
    ...booking,
...
async function handleLogin(event) {
  ...
  if (password !== STAFF_PASSWORD) {
    ...
    submitButton.textContent = "Entra";
    return;
  }
...
async function handleAvailabilitySubmit(event) {
  ...
  if (maxSeats > DEFAULT_DAILY_CAPACITY) {
    el.availabilityMessage.textContent = "La capienza massima giornaliera e di 120 coperti.";
    return;
  }
...
function renderAdmin() {
  ...
  if (el.phoneBookingPanel) {
    el.phoneBookingPanel.hidden = !canEdit();
  }
  if (el.phoneBookingForm) {
...
function renderBookingList() {
  const bookings = getBookingsByDate();
  if (!bookings.length) {
    el.bookingList.innerHTML = "<p>Nessuna prenotazione presente.</p>";
    return;
  }

  const groups = groupBookingsByDate(bookings);
  const dates = [...groups.keys()];
  if (!dates.includes(state.adminSelectedDays.bookings)) state.adminSelectedDays.bookings = null;
  const selectedDate = state.adminSelectedDays.bookings;

  el.bookingList.innerHTML = `
    ${renderDayButtons("bookings", dates, groups, (dayBookings) => {
      const people = peopleForBookings(dayBookings);
      return `${dayBookings.length} prenotazioni - ${people}/120 coperti`;
    })}
    ${selectedDate ? renderBookingDay(selectedDate, groups.get(selectedDate)) : `<p class="day-total">Scegli un giorno per vedere le prenotazioni.</p>`}
  `;

  bindDayButtons(el.bookingList, "bookings", renderBookingList);
  el.bookingList.querySelectorAll("[data-booking-day-search]").forEach((input) => {
    input.addEventListener("input", () => {
      state.bookingSearchByDate[input.dataset.date] = input.value.trim();
      renderBookingList();
    });
  });
  bindBookingActions();
...
function renderBookingDay(date, dayBookings) {
  const search = state.bookingSearchByDate[date] || "";
  const filteredBookings = filterBookingsByName(dayBookings, search);
  const people = peopleForBookings(dayBookings);
  const freeSeats = Math.max(0, getAvailabilityLimit(date) - people);

  return `
    <section class="day-group">
      <h3>${formatLongDate(date)}</h3>
      <label class="search-field">Cerca prenotazione per nome
        <input data-booking-day-search data-date="${date}" type="search" autocomplete="off" placeholder="Scrivi un nome" value="${escapeAttribute(search)}">
      </label>
      <p class="day-total">${dayBookings.length} prenotazioni - ${people}/120 coperti prenotati - ${freeSeats} disponibili</p>
      ...
      `).join("") : `<p>Nessuna prenotazione trovata per questo nome.</p>`}
    </section>
  `;
}
...
function renderProductSummary() {
  const bookings = getBookingsByDate().filter((booking) => MENU_ITEMS.some((item) => Number(booking.menu?.[item.key] || 0) > 0));
  if (!bookings.length) {
    el.productSummary.innerHTML = "<p>Nessun prodotto prenotato.</p>";
    return;
  }

  const groups = groupBookingsByDate(bookings);
  const dates = [...groups.keys()];
  if (!dates.includes(state.adminSelectedDays.products)) state.adminSelectedDays.products = null;
  const selectedDate = state.adminSelectedDays.products;

  el.productSummary.innerHTML = `
    ${renderDayButtons("products", dates, groups, (dayBookings) => {
      const totalProducts = dayBookings.reduce((total, booking) => total + MENU_ITEMS.reduce((sum, item) => sum + Number(booking.menu?.[item.key] || 0), 0), 0);
      return `${totalProducts} prodotti prenotati`;
    })}
    ${selectedDate ? renderProductDay(selectedDate, groups.get(selectedDate)) : `<p class="day-total">Scegli un giorno per vedere il riepilogo prodotti.</p>`}
  `;

  bindDayButtons(el.productSummary, "products", renderProductSummary);
}

function renderProductDay(date, dayBookings) {
  const people = peopleForBookings(dayBookings);
  const menuItems = menuItemsForDate(date);
  const totals = Object.fromEntries(menuItems.map((item) => [item.key, 0]));
  ...
  return `
    <section class="day-group">
      <h3>${formatLongDate(date)}</h3>
      <p class="day-total">${people}/120 coperti prenotati</p>
      ${menuItems.map((item) => `
...
function renderTicketList() {
  ...
  const tickets = getBookingsByDate().filter((booking) => booking.bookingType === "Online" && booking.ticketCode);

  if (!tickets.length) {
    el.ticketList.innerHTML = "<p>Nessun ticket online presente.</p>";
    return;
  }

  const groups = groupBookingsByDate(tickets);
  const dates = [...groups.keys()];
  if (!dates.includes(state.adminSelectedDays.tickets)) state.adminSelectedDays.tickets = null;
  const selectedDate = state.adminSelectedDays.tickets;

  el.ticketList.innerHTML = `
    ${renderDayButtons("tickets", dates, groups, (dayTickets) => `${dayTickets.length} ticket online`)}
    ${selectedDate ? renderTicketDay(selectedDate, groups.get(selectedDate)) : `<p class="day-total">Scegli un giorno per vedere i ticket.</p>`}
  `;

  bindDayButtons(el.ticketList, "tickets", renderTicketList);
  el.ticketList.querySelectorAll("[data-ticket-day-search]").forEach((input) => {
    input.addEventListener("input", () => {
      state.ticketSearchByDate[input.dataset.date] = input.value.trim();
      renderTicketList();
    });
  });
  bindTicketActions();
...
function renderTicketDay(date, dayTickets) {
  const search = state.ticketSearchByDate[date] || "";
  const filteredTickets = filterBookingsByName(dayTickets, search);

  return `
    <section class="day-group">
      <h3>${formatLongDate(date)}</h3>
      <label class="search-field">Cerca ticket per nome
        <input data-ticket-day-search data-date="${date}" type="search" autocomplete="off" placeholder="Scrivi un nome" value="${escapeAttribute(search)}">
      </label>
      ${filteredTickets.length ? filteredTickets.map((booking) => `
      <article class="ticket-card ${booking.ticketPresented ? "is-presented" : ""}">
        <strong>${booking.ticketCode}</strong>
        <span class="ticket-status">${booking.ticketPresented ? "Presentato" : "Da verificare"}</span>
        <span>${booking.fullName}</span>
        <span>${booking.time}</span>
        <span>${booking.people} persone - ${booking.phone}</span>
        <small>${menuSummary(booking.menu)}</small>
        ${canEdit() ? `
          <button class="ticket-toggle" type="button" data-ticket-toggle="${booking.id}" data-presented="${booking.ticketPresented ? "true" : "false"}">
            ${booking.ticketPresented ? "Segna da verificare" : "Barra come presentato"}
          </button>
        ` : ""}
      </article>
    `).join("") : `<p>Nessun ticket trovato per questo nome.</p>`}
    </section>
  `;
}

function bindTicketActions() {
  el.ticketList.querySelectorAll("[data-ticket-toggle]").forEach((button) => {
    button.addEventListener("click", () => {
      const nextValue = button.dataset.presented !== "true";
      updateBooking(button.dataset.ticketToggle, { ticketPresented: nextValue });
    });
  });
}

function getBookingsByDate() {
  return [...state.bookings].sort((a, b) => `${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`));
}

function filterBookingsByName(bookings, search) {
  const query = String(search || "").trim().toLowerCase();
  if (!query) return bookings;
  return bookings.filter((booking) => String(booking.fullName || "").toLowerCase().includes(query));
}
...
function peopleForBookings(bookings) {
  return bookings.reduce((sum, booking) => sum + Number(booking.people || 0), 0);
}

function renderDayGroups(bookings, renderContent) {
  return [...groupBookingsByDate(bookings).entries()].map(([date, dayBookings]) => `
    <section class="day-group">
      <h3>${formatLongDate(date)}</h3>
      ${renderContent(dayBookings, date)}
    </section>
  `).join("");
}

function renderDayButtons(section, dates, groups, renderMeta) {
  return `
    <div class="day-picker" aria-label="Scegli giorno">
      ${dates.map((date) => `
        <button class="day-picker-button ${state.adminSelectedDays[section] === date ? "is-selected" : ""}" type="button" data-day-section="${section}" data-day-date="${date}">
          <span>${formatLongDate(date)}</span>
          <small>${renderMeta(groups.get(date), date)}</small>
        </button>
      `).join("")}
    </div>
  `;
...
function bindDayButtons(container, section, renderAgain) {
  container.querySelectorAll(`[data-day-section="${section}"]`).forEach((button) => {
    button.addEventListener("click", () => {
      state.adminSelectedDays[section] = button.dataset.dayDate;
      renderAgain();
    });
  });
}
...
function toggleAdminSection(section) {
  ...
  if (state.adminSections[section]) {
...
function renderAdminSectionState() {
  Object.entries(state.adminSections).forEach(([section, isOpen]) => {
...
function sectionMetric(section) {
  if (section === "bookings") return state.bookings.length;
  if (section === "tickets") return state.bookings.filter((booking) => booking.bookingType === "Online" && booking.ticketCode).length;
  if (section === "products") {
    return state.bookings.reduce((total, booking) => total + MENU_ITEMS.reduce((sum, item) => sum + Number(booking.menu?.[item.key] || 0), 0), 0);
  }
  return 0;
}

function seenKey(section) {
  return `vico-seen-${section}`;
}

function markSectionSeen(section) {
  localStorage.setItem(seenKey(section), String(sectionMetric(section)));
}

function updateNotificationDots() {
  ["bookings", "products", "tickets"].forEach((section) => {
    const dot = document.querySelector(`[data-notify="${section}"]`);
    if (!dot) return;
    const seen = Number(localStorage.getItem(seenKey(section)) || 0);
    const current = sectionMetric(section);
    dot.hidden = state.adminSections[section] || current <= seen;
  });
}

function startAdminPolling() {
  if (state.adminPollId || state.localMode) return;
  state.adminPollId = window.setInterval(() => {
    if (state.currentUser) refreshData();
  }, 15000);
}

function stopAdminPolling() {
  if (!state.adminPollId) return;
  window.clearInterval(state.adminPollId);
  state.adminPollId = null;
}

function showTicketReceipt(booking) {
  if (!el.ticketReceipt || !booking) return;
  el.bookingForm.hidden = true;
  el.ticketReceipt.hidden = false;
  el.ticketReceipt.innerHTML = `
    <div class="ticket-confirmed">Prenotazione confermata</div>
    <h3>Ticket prenotazione</h3>
    <p class="ticket-warning">Fai uno screenshot di questa schermata e mostra il ticket alla cassa appena arrivi al locale per ottenere il tavolo.</p>
    <div class="ticket-code">${booking.ticketCode}</div>
    <dl>
      <div><dt>Nome</dt><dd>${booking.fullName}</dd></div>
      <div><dt>Telefono</dt><dd>${booking.phone}</dd></div>
      <div><dt>Data</dt><dd>${formatLongDate(booking.date)}</dd></div>
      <div><dt>Orario</dt><dd>${booking.time}</dd></div>
      <div><dt>Persone</dt><dd>${booking.people}</dd></div>
      <div><dt>Menu</dt><dd>${menuSummary(booking.menu)}</dd></div>
    </dl>
  `;
  el.ticketReceipt.scrollIntoView({ behavior: "smooth", block: "start" });
}

function hideTicketReceipt() {
  if (!el.ticketReceipt) return;
  el.ticketReceipt.hidden = true;
  el.ticketReceipt.innerHTML = "";
}
...
function createTicketCode() {
  ...
  return `VDM-${datePart}-${randomPart}`;
}
...
function menuItemsForDate(date) {
  return MENU_ITEMS.filter((item) => !item.dateOnly || item.dateOnly === date);
}
